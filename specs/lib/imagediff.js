...
(function (name, definition) {
  ...
  function equal (a, b, tolerance) {
    ...
    for (i = length; i-=4;) {
      var diff = (aData[i] - bData[i])*(aData[i] - bData[i]) +
                 (aData[i+1] - bData[i+1])*(aData[i+1] - bData[i+1]) +
                 (aData[i+2] - bData[i+2])*(aData[i+2] - bData[i+2]) +
                 (aData[i+3] - bData[i+3])*(aData[i+3] - bData[i+3]);
      ...
      if (typeof (document) !== UNDEFINED) {
        if(!expected) {
          this.message = function () {
            var 
              div  = get('div'),
              a    = get('div', '<div>Actual (No Expected):</div>'),
              img  = new Image();

            img.src = this.actual.toDataURL();
