...
Quintus.Audio = function(Q) {
  ...
  Q.audio.enableWebAudioSound = function() {
    ...
    Q.audio.playlist = function(soundArray,options) {
        Q.audio.playlistIndex = 0;
        playlist = soundArray;
        
        //play first sound
        if (typeof playlist[0] !== 'undefined') {
          Q.audio.play(playlist[0], options, function() {
              alert(true);
          });  
        }
  ...
  Q.audio.enableHTML5Sound = function() {
    ...
    // Play a single sound, optionally debounced 
    // to prevent repeated plays in a short time
    Q.audio.play = function(s,options,fromPlaylist) {
      var now = new Date().getTime();
      ...
      for (var i=0;i<Q.audio.channels.length;i++) {
        ...
        if (!Q.audio.channels[i]['loop'] && Q.audio.channels[i]['finished'] < now) {	
          ...
          if(fromPlaylist) {
            Q.audio.channels[i].addEventListener('ended', callback());    
          }
  ...
  playNextTitle = function(options) {
      //increase playlist index
      Q.audio.playlistIndex++;
      
      //if next sound exists, play it
      if(typeof playlist[Q.audio.playlistIndex] !== 'undefined') {
          Q.audio.play(playlist[Q.audio.playlistIndex], options, true);
      }
