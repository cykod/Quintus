...
var quintusTMX = function (Quintus) {
	...
	Quintus.TMX = function (Q) {
		...
		Q._tmxExtractAssetName = function (result, cutPath) {
		...
		Q._tmxExtractSources = function (asset, cutPath) {
		...
		Q.loadTMX = function (files, callback, options) {
			...
			Q.load(files, function () {
				...
				if (additionalAssets.length > 0) {
					...
					Q.load(additionalAssets, function () {
						var additionalAssetsImgs = [];
						Q._each(additionalAssets, function (additionalAsset) {
							var sources = Q._tmxExtractSources(Q.asset(additionalAsset));
							additionalAssetsImgs = additionalAssetsImgs.concat(sources);
						});
						if (additionalAssetsImgs.length > 0) {
							Q.load(additionalAssetsImgs, callback, options);
						} else {
							callback();
						}
					}, options);
		...
		Q._tmxLoadTilesets = function (tilesets, tileProperties) {
			...
			for (var t = 0; t < tilesets.length; t++) {
				...
				var source = Q._tmxExtractAssetName(tileset);
				var gid = attr(tileset, "firstgid");

				if (source) {
					var asset = Q._isString(source) ? Q.asset(source) : source;
					var tilesets = asset.getElementsByTagName("tileset");
					if (tilesets[0]) {
						tileset = tilesets[0];
					}
				}
				...
				var sheetName = attr(tileset, "name"),
		...
		Q._tmxProcessTileLayer = function (stage, gidMap, tileProperties, layer) {
			...
			if (isNaN(tileLayerProperties['type']) && !Q._isUndefined(Q[tileLayerProperties['type']])) {
				tileLayerProperties['type'] = Q[tileLayerProperties['type']];
			}

			if (tileLayerProperties['collision'] && tileLayerProperties['collision'].toLowerCase() === 'true') {
