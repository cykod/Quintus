...
/** Quintus module for adding 2D sprites to the engine
 *  @namespace
 *  @name Quintus.Sprites
 *
 *  @example
 *  // Add in the sprite module
...
Quintus.Sprites = function(Q) {
  ...
  Q.Class.extend("SpriteSheet",  {
    /** @constructs
     *  @name Quintus.Sprites.SpriteSheet
     *
     * @param {string} name - name of the sprite sheet
     * @param {string|Object} asset - the asset to associate with the sprite sheet
     * @param {Object} options - options hash
     *
     * @example
     * // The default values of the options hash are shown below
     * new Q.SpriteSheet("Player","player.png", {
    ...
    /** Returns the starting x location of a specific frame in the sprite sheet
     *  @method fx 
     *  @memberOf Quintus.Sprites.SpriteSheet#
     *  
     *  @param {Integer} frame - the frame to generate the x location for
     */
    fx: function(frame) {
      return Math.floor((frame % this.cols) * this.tilew + this.sx);
    },

    /** Returns the starting y location of a specific frame in the sprite sheet
     *  @method fy 
     *  @memberOf Quintus.Sprites.SpriteSheet#
     *  
     *  @param {Integer} frame - the frame to generate the y location for
     */
    fy: function(frame) {
      return Math.floor(Math.floor(frame / this.cols) * this.tileh + this.sy);
    },

    /** Draw a frame of the Sprite sheet at a location on the passed-in context
     *  @method draw
     *  @memberOf Quintus.Sprites.SpriteSheet#
     *  
     *  @param {CanvasContext} ctx - the destinatio context
     *  @param {Number} x - the x location on the context
     *  @param {Number} y - the y location on the context
     *  @param {Integer} frame - the frame to draw
     *
     */
  ...
  /** A hash of the currently registered sprite sheets - normally accessed via Q.sheet
   *
   *  @name sheets
   *  @property {Object} sheets
   *  @memberOf Quintus.Sprites
   */
  Q.sheets = {};

  /** Either returns a sprite sheet of a given name or create a new sprite sheet
   *
   *  @method sheet
   *  @memberOf Quintus.Sprites
   *
   *  @param {string} name - name of spritesheet to create or return
   *  @param {string} [asset] - asset name, if creating a sprite sheet
   *  @param {Object} [options] - options to create the sprite sheet - see {@link Quintus.Sprites.SpriteSheet}
   *
   *  @example
   *  var Q = Quintus().include("Sprites");
   *
   *  // Create a new sprite sheet
   *  Q.sheet("bricks","bricks.png", { tilew: 32, tileh: 32 });
   *
   *  // Return the created bricks sprite sheet
   *  var sheet = Q.sheet('bricks');
   */
