...
Quintus.Scenes = function(Q) {
  ...
  Q.Stage = Q.GameObject.extend({
    ...
    /*  
      ...
      and name that object "Player".  Then, go back to your collision layer and set the
      "objectLayer" property to the name of the object layer you just created.
    */
    loadTMX: function(dataAsset) {
      ...
      if(objectLayerName) {
        ...
        var player = doc.documentElement.querySelector("[name='" + objectLayer + "']").querySelector("[name=Player]");
        console.log(player);
        console.log(player.getAttribute("x") + ", " + player.getAttribute("y"));
        if(player) {
          var p = this.insert(new Q.Player({ 
            x: parseInt(player.getAttribute("x")), 
            y: parseInt(player.getAttribute("y"))            
          }));
          
          this.add("viewport").follow(p);
        }
      }
    },
