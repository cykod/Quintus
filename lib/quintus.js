...
var Quintus = function Quintus(opts) {
  ...
  // class level methods.
  //
  // Classes can be created by calling `Q.Class.extend({ .. })`, although most of the time
  ...
  // by favoring composition over inheritance) and can be added and removed on the fly at runtime.
  //
  ...
  // Returns the `Q` object from the `Quintus()` factory method. Create awesome games. Repeat.
  return Q;
}

// Finally, add in the `requestAnimationFrame` shim, if necessary. Does nothing 
