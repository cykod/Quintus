  ...
  */
  Q.setup = function(id, options) {
    ...
    if(options.scaleToFit == true) {
      ...
      var gameRatio = Q.width/Q.height;

      var ratio = gameRatio < winRatio ? height/Q.height : width/Q.width;
      Q.el.style.webkitTransform = 'scale('+ ratio + ','+ ratio + ')';
      Q.el.style.MozTransform = 'scale('+ ratio + ','+ ratio + ')';
      Q.el.style.msTransform = 'scale('+ ratio + ','+ ratio + ')';

      if(gameRatio < winRatio) {
        Q.el.style.webkitTransformOrigin = 'top';
        Q.el.style.MozTransformOrigin = 'top';
        Q.el.style.msTransformOrigin = 'top';  //not tested!
        //no idea for Opera!

        if(window.innerWidth < Q.width) {
          var leftPos = (window.innerWidth - Q.width)/2;          
          Q.el.style.left = leftPos+'px';
        }        
        
        Q.el.parentNode.style.height = (Q.height * ratio) + 'px';
        document.body.style.overflow = 'hidden';
      }
      else {    
        Q.el.style.webkitTransformOrigin = 'left';
        Q.el.style.MozTransformOrigin = 'left';
        Q.el.style.msTransformOrigin = 'left';  //not tested!
        //no idea for Opera!      

        var topPos = (window.innerHeight - Q.height)/2;          
        Q.el.style.top = topPos+'px';
        Q.el.parentNode.style.marginLeft = '0px';
        Q.el.parentNode.style.marginRight = '0px';
        document.body.style.overflow = 'hidden';
      }
    }
