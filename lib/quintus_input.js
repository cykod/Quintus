...
Quintus.Input = function(Q) {
  ...
  Q.component("platformerControls", {
    ...
    step: function(dt) {
      ...
      // For now, only follow along the slope if there's exactly one collision.
      if(p.collisions !== undefined && p.collisions.length == 1 && (Q.inputs['left'] || Q.inputs['right'])) {
        collision = p.collisions[0];
        
        // Don't climb up walls.      
        if(collision.normalY > -.3 && collision.normalY < .3) {        
          collision = null;
        }
      }
